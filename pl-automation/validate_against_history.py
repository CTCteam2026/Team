...
CASES = [
 dict(key="LCT", name="LCT / Marine Recreation Assn 2026",
      ...
      scope=["PLANNING, TIMELINE & COMMUNICATIONS:", "VENUE MANAGEMENT:",
 ...
 dict(key="WTUI", name="WTUI 2027",
      ...
      scope=["PLANNING, TIMELINE & COMMUNICATIONS:", "VENUE SOURCING:",
...
def run_case(case, verbose=False):
    ...
    for r in range(SCOPE_FIRST, SCOPE_LAST + 1):
        label = sc.cell(row=r, column=2).value or ""
        sc.cell(row=r, column=INC_COL,
                value="Yes" if any(label.startswith(p) for p in case["scope"]) else "No")
    wb.save(path)
    ...
    if verbose:
        s = v["SCOPE"]
        for r in range(4, 24):
            if (s.cell(row=r, column=17).value or 0) > 0:
                print(f"    {s.cell(row=r, column=16).value:<40}"
                      f"{s.cell(row=r, column=17).value:>6.0f} hrs")
    return dict(pre_hrs=p["D43"].value, pre_client=p["E43"].value,
                on_hrs=p["D68"].value, on_client=p["E68"].value,
